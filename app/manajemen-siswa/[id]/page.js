...
export default async function Page({ params }) {
  let data = null;
  const { id } = params;
  await httpCall("POST", "http://" + headers().get("host") + API.USERS_GET_DETAIL_SISWA, { id }, { cookie: headers().get("cookie") })
    .then((res) => {
      if (res.status === 200) {
        data = res.data;
      }
...
export async function generateMetadata({ params }) {
  let title = "Siswa";
  const { id } = params;
  await httpCall("POST", "http://" + headers().get("host") + API.USERS_GET_NAMA_SISWA, { id }, { cookie: headers().get("cookie") })
    .then((res) => {
      if (res.status === 200) {
        title = res.data.nama;
      }
